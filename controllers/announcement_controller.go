...
//+kubebuilder:rbac:groups=community.zufardhiyaulhaq.com,resources=announcements,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=community.zufardhiyaulhaq.com,resources=announcements/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=community.zufardhiyaulhaq.com,resources=announcements/finalizers,verbs=update

func (r *AnnouncementReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := log.FromContext(ctx)
	log.Info("Start Announcement Reconciler")

	announcement := &communityv1alpha1.Announcement{}
	err := r.Client.Get(ctx, req.NamespacedName, announcement)
	if err != nil {
		return ctrl.Result{}, nil
	}

	if announcement.Status.Status == "send" {
		log.Info(fmt.Sprintf("announcement %s has been send for all communities, skip reconcile", announcement.Name))
		return ctrl.Result{}, nil
	}

	announcementMessage := announcement.Spec.Spec.ToMessageAnnouncement()
	...
	for _, community := range communities.Items {
		if community.Namespace == req.Namespace && operatorHelper.StringInSlice(community.Name, announcement.Spec.Community) {
			filteredCommunities = append(filteredCommunities, community)
		}
	}

	if len(filteredCommunities) != len(announcement.Spec.Community) {
		return ctrl.Result{}, fmt.Errorf("cannot find community object in announcement.Spec.Community")
	}

	log.Info("Populate announcement status")
	if announcement.Status.Details.Community == nil {
		announcement.Status.Details = communityv1alpha1.AnnouncementStatus_Details{
			Community: map[string]communityv1alpha1.AnnouncementStatus_Details_Community{},
		}

		for _, community := range filteredCommunities {
			communityDetails := map[string]communityv1alpha1.AnnouncementStatus_Details_Community{
				community.Name: {
					Handler: communityv1alpha1.AnnouncementStatus_Details_Community_Handler{
						Telegram: map[string]communityv1alpha1.AnnouncementStatus_Details_Community_Handler_Telegram{},
						Twitter:  map[string]communityv1alpha1.AnnouncementStatus_Details_Community_Handler_Twitter{},
					},
				},
			}

			for _, telegram := range community.Spec.SocialMedia.Telegram {
				telegramDetails := map[string]communityv1alpha1.AnnouncementStatus_Details_Community_Handler_Telegram{
					telegram: {
						Status: "",
					},
				}
			...
			for _, twitter := range community.Spec.SocialMedia.Twitter {
				twitterDetails := map[string]communityv1alpha1.AnnouncementStatus_Details_Community_Handler_Twitter{
					twitter: {
						Status: "",
					},
				}
			...
			for key, value := range communityDetails {
				announcement.Status.Details.Community[key] = value
			}
		}
	}
	...
	for _, community := range filteredCommunities {
		...
		*/
		...
		for _, telegramHandler := range filteredtelegramHandlers {
			if announcement.Status.Details.Community[community.Name].Handler.Telegram[telegramHandler.Name].Status == "send" {
				log.Info(fmt.Sprintf("skip sending announcement on community %s on telegram handler %s", community.Name, telegramHandler.Name))
				continue
			}
			...
			log.Info(fmt.Sprintf("sending announcement on community %s on telegram handler %s", community.Name, telegramHandler.Name))

			handler := operatorHandler.NewTelegramHandler(client)
			err = handler.SendMessage(announcementMessage)
			if err != nil {
				return ctrl.Result{}, err
			}

			announcement.Status.Details.Community[community.Name].Handler.Telegram[telegramHandler.Name] = communityv1alpha1.AnnouncementStatus_Details_Community_Handler_Telegram{
				Status: "send",
			}
		...
		*/
		...
		for _, twitterHandler := range filteredtwitterHandlers {
			if announcement.Status.Details.Community[community.Name].Handler.Twitter[twitterHandler.Name].Status == "send" {
				log.Info(fmt.Sprintf("skip sending announcement on community %s on twitter handler %s", community.Name, twitterHandler.Name))
				continue
			}
			...
			log.Info(fmt.Sprintf("sending announcement on community %s on twitter handler %s", community.Name, twitterHandler.Name))
			...
			announcement.Status.Details.Community[community.Name].Handler.Twitter[twitterHandler.Name] = communityv1alpha1.AnnouncementStatus_Details_Community_Handler_Twitter{
				Status: "send",
			}
	...
	announcement.Status.Status = "send"
	err = r.Client.Status().Update(context.TODO(), announcement)
	if err != nil {
		return ctrl.Result{}, err
	}
...
func (r *AnnouncementReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&communityv1alpha1.Announcement{}).
		Complete(r)
}
